...
MyApplet.prototype = {
  ...
  updateMenu: function() {
    ...
    try {
      let [res, out, err, status] = GLib.spawn_command_line_sync('cat .ssh/config');
      if(out.length!=0) {
        //let debugmenu = new PopupMenu.PopupSubMenuMenuItem("DEBUG");
        //this.menu.addMenuItem(debugmenu);
        let entries = out.toString().split("\n");
        let submenu = null;
        for(let i=0; i<entries.length; i++) {
          let entry = entries[i];
          if(entry != ""){
            if(entry.substring(0, 4) == "### ") {
              let subname = entry.replace("### ", "");
              submenu = new PopupMenu.PopupSubMenuMenuItem(subname);
              this.menu.addMenuItem(submenu);
            }
            else if(entry.substring(0, 5)== "Host ") {
              let hostname = entry.replace("Host ", "");
      ...
      /*
        let subs = out.toString().split("\n");
        for(let j=0; j<subs.length; j++) {
          let sub = subs[j];
          if(sub != "") {
            let subname = sub.replace("### ", "");
            let submenu = new PopupMenu.PopupSubMenuMenuItem(subname);
